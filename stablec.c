...
double range = 0.0000000001; // criteria for convergence
...
int main(){
    ...
    void write_samp(double Ns[N_sample][S]);
    void write_traj_judge(double** Nt, int sample);
    int single_time_check(double** Nt, int t, int back);
    void write_train(double** Nt, FILE *tfile);
    ...
    for (int b = 0; b < S; b++) {
        Nt[b] = (double *)malloc(N * sizeof(double));
    }
...
int evolve( double** Nt, double a[S][S]){
  ...
  // generate the initial values of each species
    ...
    for(int s=0; s<S; s++){
        Nt[s][0] =( (double)rand() / RAND_MAX - 0.5)*0.0000001 + Ni[s];
        if(Nt[s][0] < 0){
            Nt[s][0] *= -1;
        }
    }
    ...
    for (int s = 0; s < S; s++){
        ...
        //if(Ni[s]){
            Nt[s][0] = (convertBytesToDouble(random_bytes)-0.5) * 0.0000001 + Ni[s];
        //}
        if(Nt[s][0]<0){
            Nt[s][0] *= -1;
        }
    ...
    */
    ...
    for(; t<N; t++){
        ...
        if(t == RLEN - 1){
            char rname[50];
            sprintf(rname, "r_S%d_T%.0e_del%d_r%.1Le.csv", S, (double)T, delay, r);
...
void write_traj(double** Nt){
    ...
    for (int k = 0; k<interval; k++){
        for (int l = 0; l<S; l++){
            fprintf(dfile, "%.15Lf", Nt[l][(int)(N-interval+k)]);
            if(l<S-1){
                fprintf(dfile, ",");
            }
        }
