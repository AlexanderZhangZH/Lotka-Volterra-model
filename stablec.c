// This version of evolve() will OUTPUT FILE!
# include<stdlib.h>
# include<stdio.h>
# include<math.h>
# include<time.h>
...
long double sig = 1; //variance of a_ij
long double mygamma = 0; //covariance factor of a_ij and a_ji
long double r; //= 2.5; // uniform growth rate of species.
...
int main(){
    ...
    if(pfile == NULL){
        printf("Unable to find initial value file\n");
        return 0;
    }
    char comma;
    char ret = ',';
    for(int i = 0; i<S; i++){
        fscanf(pfile, "%Lf", &Ni[i]);
        fscanf(pfile, "%c", &comma);
    }
    fclose(pfile);
    ...
    if(converge(Nt)){
        ...
        printf("possible pattern: %d\n", pattern);
    }else{
        int dif = 0;
        for(int s = 0; s<S; s++){
            if(Nt[s][(int)(N-1)] - Ni[s] >= range || Ni[s] - Nt[s][(int)(N-1)] >= range){
                dif++;
            }
        }
        if(dif){
...
void pairgenerate(long double mean, long double variance, long double correlation, long double* result1, long double* result2) {
    ...
    long double z1 = sqrt(-2.0 * log(u1)) * cos(2.0 * M_PI * u2);
    long double z2 = sqrt(-2.0 * log(u1)) * sin(2.0 * M_PI * u2);
...
int matrix_generate(long double a[S][S], long double sig){
    ...
    sprintf(aname, "a_S%d|mu%.0Lf|sig%.0Lf|gamma%.0Lf.csv", S, mu, sig, mygamma);
    ...
    int choice = 1; // parameter to show the choice
...
int evolve( long double** Nt, long double a[S][S]){
  ...
  // generate the initial values of each species
   /*
    ...
    for(s=0; s<S; s++){
        Nt[s][0] = (double)rand() / RAND_MAX; // random initial value between 0 and 1
    }
    */
    int fd;
    ...
    for(; t<N; t++){
        for(i=0; i<S; i++){
            if(Ext[i]){
                ...
                for(j=0; j<S; j++){
                    if(Ext[j]){
                        sum += a[i][j]*Nt[j][(int) (t-1-(int) (delay))]*Nt[i][(int) (t-1)];    
                    
                    }
                }
                // evolution
                Nt[i][(int) t] = Nt[i][(int) (t-1)] + r*(-sum + Nt[i][(int) (t-1)]);
                // Kill the program when abnormal behaviour of N_i is found and report the situation
                if (Nt[i][(int) t]>100000){ 
                    //printf("blow up \n");
                    return -1;
                }
                if (Nt[i][(int) t]<range){
...
int converge(long double** Nt){
    ...
    for (int s = 0; s<S; s++){
        ...
        if(Nt[s][(int)(N-1)] - Nt[s][(int)(N-2 - td)]>=range || Nt[s][(int)(N-2 - td)] - Nt[s][(N-1)] >= range){// to see if the change on abundance of species i over the interval is within the range set before
            status++;
        }  
    }}
    return status;
...
int single_time_check(long double** Nt, int t, int back){
    int match = 0;
    for (int s = 0; s < S; s++){
        if(Nt[s][(int)t] - Nt[s][(int)(t - back)] < range && Nt[s][(int)(t - back)] - Nt[s][(int)t] < range){
            match++;
        }
    }
    if(match == S){
